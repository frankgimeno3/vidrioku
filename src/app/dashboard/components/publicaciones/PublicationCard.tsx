...
const PublicationCard: FC<PublicationCardProps> = ({ publicacion }) => {
    ...
    useEffect(() => {
        if (publicacion.isLiked == true) { setIsPublicationLiked(true) }
        if (publicacion.isLiked == false) { setIsPublicationLiked(false) }
    }, [publicacion]);

    const toggleLike = async () => {
        try {
            const docRef = doc(db, "publicaciones", publicacion.id);
            const publicationDoc = await getDoc(docRef);
            ...
            if (publicationDoc.exists()) {
                ...
                const updatedData = {
                    ...
                    isLiked: !isPublicationLiked,
                };
                setIsPublicationLiked(!isPublicationLiked)
                ...
                console.error('El documento de la publicacion no existe');
            }
        } catch (error) {
    ...
    return (
        <div className='text-left rounded-lg shadow-lg p-1 text-gray-600 bg-gray-100 shadow px-5 mb-5'>
            <div className='flex flex-row m-5 items-end'>
                ...
                <div className='flex flex-col justify-bottom m-5'>
                    <p className=' font-bold'>{publicacion.titulo}</p>
                    <p className='font-bold text-sm text-gray-500 '>{publicacion.autor}</p>
                </div>

            </div>

            <p className='flex-1 text-sm justify-end pl-5'>Fecha de publicación: </p>
            <p className='px-5'>{publicacion.contenido}</p>
            <div className='px-auto px-5 py-5' >
                {isPublicationLiked == false &&
                    <div className='w-8 h-8' onClick={() => toggleLike()}>
                ...
                {isPublicationLiked == true &&
                    <div className='w-8 h-8' onClick={() => toggleLike()}>
                        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M2 9.1371C2 14 6.01943 16.5914 8.96173 18.9109C10 19.7294 11 20.5 12 20.5C13 20.5 14 19.7294 15.0383 18.9109C17.9806 16.5914 22 14 22 9.1371C22 4.27416 16.4998 0.825464 12 5.50063C7.50016 0.825464 2 4.27416 2 9.1371Z" fill="#1C274C"></path> </g></svg>
                    </div>
                }
            </div>
